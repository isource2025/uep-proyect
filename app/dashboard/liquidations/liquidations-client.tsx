...
import { calculateLiquidation, updateLiquidation, fetchLiquidationData } from "./actions";
...
export default function LiquidationsClientPage({ initialData }: LiquidationsClientPageProps) {
  ...
  const handleRunCalculate = async (rcId: number) => {
    ...
    try {
      ...
      // Scroll to the bottom of the scrollable main container once generated to see result
      setTimeout(() => {
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between border-t border-emerald-500/10 mt-4 pt-4 gap-4 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {[...Array(totalPendingPages)].map((_, index) => {
                    ...
                    const isNear = Math.abs(pageNum - safePendingPage) <= 1;
                    const isFirstOrLast = pageNum === 1 || pageNum === totalPendingPages;

                    if (isCurrent || isNear || isFirstOrLast) {
                      return (
                        <Button
                          key={pageNum}
                    ...
                    if (pageNum === 2 || pageNum === totalPendingPages - 1) {
                      return <span key={pageNum} className="px-1 text-muted-foreground select-none">...</span>;
                    }
